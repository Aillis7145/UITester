...
> **ข้อสอบมีสองชุด คนละขนาด คนละที่มา** — ท้ายบท (`?set=unit`, ไม่เกิน 20 ข้อ, เข้าจากหน้าวิดีโอ,
> มาจากไฟล์ Word จริง) กับ ปลายคอร์ส (`?set=course`, **50 ข้อ**, เข้าจากแท็บแบบทดสอบ, เป็นที่มาของวุฒิบัตร)
> ชุดปลายคอร์สเป็นข้อสอบจำลองที่เขียนขึ้นเอง มีสองภาษา — CEFR ได้ชุดอังกฤษ HSK ได้ชุดจีน
> และพก "ใบคำตอบ" ของตัวเองมาด้วย หน้าผลสอบจึงมีคะแนนจริงให้ดู (37/44) และปุ่มดูวุฒิบัตรโผล่จริง
> `quizFor(node, setId)` เป็นจุดเดียวที่ตัดสินว่าได้ชุดไหน ([docs/authoring.md](docs/authoring.md))

> **แท็บแบบทดสอบมีสามด่าน** — เลือกวิชา → เตรียมพร้อม (ตรวจเสียง) → ข้อสอบ
> เดิมกดแท็บแล้วเข้าห้องสอบทันทีทั้งที่ยังไม่รู้ว่าจะสอบวิชาอะไร และเวลาเริ่มเดินไปแล้ว
> หน้าเลือกวิชาแสดงเฉพาะวิชาที่สอบวัดระดับจริง (`EXAM_PROJECT_IDS` — CEFR + HSK รวม 7 คอร์ส)
> ตัวเลขบนการ์ดอ่านจากชุดข้อสอบจริงผ่าน `quizFor()` การ์ดกับห้องสอบจึงไม่มีทางบอกคนละเลข
> หน้าเตรียมพร้อมทดสอบลำโพงด้วยเสียงจริง ส่วนไมโครโฟนเป็นการจำลองที่เขียนบอกไว้ตรงๆ
> เพราะ `getUserMedia` ใช้ไม่ได้บน http ในวงแลน ([docs/authoring.md](docs/authoring.md))

> **หน้าความคืบหน้า — ทุกวิชา ทุกคอร์ส ทุกบท ในหน้าเดียว** ข้อกำหนดคือ *ไม่ต้องกดหลายครั้ง*
> จึงไม่มี `<details>` / accordion / tab / tooltip เลยสักตัว ทุกตัวเลขอยู่บนหน้าตั้งแต่โหลดเสร็จ
> เซลล์ของแต่ละบทไล่ **ทุกรอบที่ทำแบบทดสอบท้ายบท** บรรทัดละรอบ (วันที่ · %) เรียงเก่าไปใหม่
> สีของเซลล์มีสองสี เส้นแบ่งคือเกณฑ์ผ่าน 60% เท่ากับที่ข้อสอบใช้จริง และมาจากรอบล่าสุดเพราะนั่นคือสถานะปัจจุบัน
> 85 บทที่จบ 251 รอบ (สูงสุด 5 รอบต่อบท) การ์ดโชว์สามรอบล่าสุด ที่เหลือกดเปิด popup ดูได้ครบ
> มีชิปกรองหลักสูตรด้านบนเหมือนหน้าคอร์ส และวุฒิบัตรออกให้เมื่อเรียนครบทุกบท + ทำแบบทดสอบท้ายบทครบเท่านั้น
> คอร์สที่ยังไม่เริ่มบีบเหลือบรรทัดเดียว ไม่วาดเซลล์เทายาว ([docs/authoring.md](docs/authoring.md))

> **ประวัติการเรียน + ใบประกาศนียบัตร** — สองหน้าใหม่ที่คิดจากต้นไม้เนื้อหาจริง ไม่มี id เขียนตายสักตัว
> ประวัติจัดกลุ่มตามวันแบบประวัติของเบราว์เซอร์ ค้นหาได้ และทุกแถวกดกลับไปที่เดิมผ่าน `targetFor` ตัวเดียวกับทั้งแอป
> ใบประกาศวาดด้วย `<canvas>` ตัวจริง ดาวน์โหลดเป็น PNG 3200×2240 ที่ใช้สีของธีมที่กำลังดูอยู่
> เวลาในข้อมูลเป็นค่าคงที่ ไม่ใช่ `Date.now()` — ไม่งั้นภาพถ่ายหน้าจอจะเปลี่ยนเองทุกวัน ([docs/authoring.md](docs/authoring.md))
...
│   ├─ records.js      ประวัติการเรียน + ใบประกาศ คิดจากต้นไม้ ไม่มี id เขียนตาย
│   ├─ courseExam.js   ข้อสอบปลายคอร์ส 50 ข้อ สองภาษา — ข้อสอบจำลอง ตั้งใจให้แทนที่ทั้งไฟล์
