...
| | `p1` ศูนย์การเรียนรู้ AI | `p2` ติวสอบภาษาจีน | `p3` ภาษาอังกฤษ | `p4` ห้องเรียนออนไลน์มัธยม |
|---|---|---|---|---|
| ชั้น 1 | `course` — **วิชา** | `course` — **ระดับ** | `course` — **ระดับ** | `course` — **ชั้นปี** |
| ชั้น 2 | `module` — **หน่วย** | `module` — **บท** | *(ข้าม)* | *(ข้าม)* |
| ชั้น 3 | `lesson` — **หัวข้อ** | *(ข้าม)* | `lesson` — **ตอน** | *(ข้าม)* |
| **หน้าไล่ระดับ** | **2** | **1** | **1** | **0** |
...
`p4` เป็น**กรณีตื้นที่สุด**: ชั้นเดียวล้วน กดชั้นปีปุ๊บถึงวิดีโอปั๊บ ไม่ผ่านหน้าไล่ระดับเลย
ถ้าโค้ดตรงไหนแอบสมมติว่า "ต้องมีชั้นกลางอย่างน้อยหนึ่งชั้น" จะพังที่โครงการนี้ก่อนเพื่อน

**หน้าไล่ระดับ = `levels.length - 1`** เพราะกล่องชั้นล่างสุดตัดตรงไปหน้าสื่อ
ซึ่งมีลิสต์สื่อทั้งกล่องอยู่ข้างอยู่แล้ว การมีหน้าลิสต์คั่นอีกหน้าจึงเป็นการกดซ้ำเปล่าๆ
...
ซึ่งตอบโดย `src/mock/projects.js` และปุ่มทุกใบในแอปเรียก `targetFor(node, project)`
ตัวเดียวใน `src/screens/nav.js` — นี่คือจุดเดียวในโปรเจคที่รู้ว่าอะไรพาไปหน้าไหน

> `npm run verify` เดินครบทุกโครงการแล้วยืนยันว่า **จำนวนหน้า `browse` ที่ผ่าน = `levels.length - 1`**
> โดยอ่านค่าคาดหวังจาก `projects.js` ไฟล์เดียวกับที่แอปอ่าน จึงผ่านโดยบังเอิญไม่ได้
> และมีเช็คกันไว้ด้วยว่าโครงการตัวอย่างต้องลึกไม่เท่ากันจริง ไม่งั้นการทดสอบนี้ไม่พิสูจน์อะไรเลย
...
> สามข้อบนคือเส้นทางของ**นักพัฒนา** ส่วนขั้นตอนของ**ผู้ดูแลที่สร้างเนื้อหา**
